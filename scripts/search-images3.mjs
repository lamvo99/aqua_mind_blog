...
const searches = {
  'nitrogen-cycle': ['aquarium water testing', 'water test aquarium kit', 'fish tank test water'],
  'water-change': ['siphon aquarium', 'aquarium fish net', 'bucket aquarium'],
  'water-change-frequency': ['aquarium fish tank water', 'freshwater tank fish', 'aquarium thermometer'],
}
  ...
  for (const term of terms) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        const picks = (good.length ? good : results).slice(0, 5)
